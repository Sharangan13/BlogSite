          ...
          <form className="flex flex-col gap-5" onSubmit={handleSubmit} encType="multipart/form-data">
            {[{ label: 'Title', name: 'title', type: 'text', placeholder: 'Give your story a title' }, { label: 'Author Name', name: 'author', type: 'text', placeholder: 'Your name' }].map(({ label, name, type, placeholder }) => (
              <div key={name} className="flex flex-col gap-2">
                ...
                <input name={name} type={type} required value={fields[name]} onChange={onChange} placeholder={placeholder} className={inputCls} />
              </div>
            ))}
            ...
            <div className="flex flex-col gap-2">
              ...
              <textarea name="body" rows="10" required value={fields.body} onChange={onChange} placeholder="Tell your story…" className={`${inputCls} resize-y min-h-[140px]`} />
            </div>
            ...
            <button type="submit" disabled={btnDisable || loading} className="w-full py-3 px-6 bg-blue-600 text-white border-none rounded-md text-sm font-semibold mt-2 hover:bg-blue-700 disabled:opacity-60 disabled:cursor-not-allowed transition-colors">
