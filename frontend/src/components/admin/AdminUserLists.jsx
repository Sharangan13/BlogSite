...
function UserRow({ user, onDeleteClick, editPath, isSelf }) {
  return (
    <tr className="hover:[&>td]:bg-gray-50">
      <td className="px-4 sm:px-5 py-4 text-sm text-gray-600 border-b border-gray-200">{user.name}</td>
      <td className="px-4 sm:px-5 py-4 text-sm text-gray-600 border-b border-gray-200">{user.email}</td>
...
function AdminTable({ title, rows, loading, onDeleteClick, currentUserId, getEditPath }) {
  return (
    <div className="flex min-h-[calc(100vh-64px)]">
      <Sidebar />
      <div className="flex-1 px-4 sm:px-8 py-6 sm:py-10 overflow-x-auto">
        <h1 className="text-2xl font-bold mb-6 sm:mb-8 text-gray-900" style={{ fontFamily: 'var(--font-display)' }}>{title} ({rows.length})</h1>
        {loading ? <Loader /> : (
...
export function UserList() {
  ...
  const [dialog, setDialog] = useState({ open: false, userId: null, name: '' });

  const handleDeleteClick = (user) => setDialog({ open: true, userId: user._id, name: user.name });
  const handleConfirmDelete = () => {
    dispatch(AdminDeleteUser(dialog.userId));
    setDialog({ open: false, userId: null, name: '' });
  };
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        message={`"${dialog.name}" will be permanently removed.`}
        confirmText="Delete"
        onConfirm={handleConfirmDelete}
        onCancel={() => setDialog({ open: false, userId: null, name: '' })}
      />
      <AdminTable
        title="Bloggers"
        rows={bloggers}
