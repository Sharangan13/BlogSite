...
export const getBlogs = (keyword) => async (dispatch) => {
    ...
    try {
        ...
        dispatch(blogsFail(error.response?.data?.message || 'Failed to load blogs'));
    }
};
...
export const getMyBlogs = () => async (dispatch) => {
    try {
        ...
        dispatch(myBlogsFail(error.response?.data?.message || 'Failed to load your blogs'));
    }
};

export const adminGetBlogs = (keyword) => async (dispatch) => {
    ...
    try {
        ...
        dispatch(adminBlogsFail(error.response?.data?.message || 'Failed to load blogs'));
    }
};
